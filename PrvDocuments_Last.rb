...
#*************
    ...
    #++++++++++++++
        ...
        print "? "
        selection = CLI::UI.ask("Votre choix : ").to_i
        ...
        puts    ModeleUI.ui_info("Source: conversion - Fichier: #{fichier}")
        return  true
    end
...
#*****************
    ...
    #++++++++++++++++
        ...
        ecr_properties['Dossier']       = std.relation1(arr_properties['Dossier'][0])   unless arr_properties['Dossier'][0].nil?
        ecr_properties['Tags']          = std.relation1(arr_properties['Tags'][0])      unless arr_properties['Tags'][0].nil?
        ecr_properties['Type']          = std.relation1(arr_properties['Types'][0])     unless arr_properties['Types'][0].nil?
        ecr_properties['Emetteur']      = std.relation1(arr_properties['Emetteur'][0])  unless arr_properties['Emetteur'][0].nil?
    ...
    #++++++++++++++++
        ...
        puts    ModeleUI.ui_info("Référence::Lecture:#{reference} - #{arr_properties['Statut']}")

        unless  index.include?(reference)
            index.push(reference)
            ecr_properties  = {}
            ecr_properties['Référence']     = std.title(reference)
            ecr_properties['Domaine']       = std.select("DOC")
    ...
    #    puts    ModeleUI.ui_info("Props_entrée::#{arr_properties}")
    #    puts    ModeleUI.ui_info("Props_sortie::#{ecr_properties}")
        return  ecr_properties
    end
...
#-----------------------------
    ...
    #******
        ...
        ecr_properties  = {}
        #
        ModeleUI.ui_step("Logique") do
            LOG.info("--Les Index")
            ModeleUI.ui_spin("Les Index") {arr_index = lesIndex()}
            LOG.info("----#{arr_index.size} index")
        #
            LOG.info("--Choix de la source")
            source          = Source.new()
            PRMS[:clsource] = source
            ModeleUI.ui_spin("La Source") {PRMS[:source] = source.choixSource()}
        #
            ...
            when    'fichier'
                ModeleUI.ui_spin("Lecture") {arr_pages = lecture.chargeFichier()   if PRMS[:source_type] == 'fichier'}
            when    'dossier'
                ModeleUI.ui_spin("Lecture") {arr_pages = lecture.chargeDossier()   if PRMS[:source_type] == 'dossier'}
            when    'conversion'
                ModeleUI.ui_spin("Lecture") {arr_pages = lecture.chargeConversion()   if PRMS[:source_type] == 'conversion'}
            end
        ...
        #
            ...
            when    'dossier'
            when    'conversion'
                ModeleUI.ui_spin("Conversion - Ecriture") do
                    arr_pages.each do |page|
                        if PRMS[:notion_fichier] == "Mes Documents"
                            ModeleUI.ui_step("Conversion_1") {ecr_properties = conversion.conversionMes(page)}
                        else
                            ModeleUI.ui_step("Conversion_X") {ecr_properties = conversion.conversionDoc(page, arr_index)}
                        end
                        ModeleUI.ui_step("Ecriture") {ecriture.ajoutDossier(ecr_properties) unless ecr_properties.nil?}
                    end
                end
            end
    ...
    #++++++++++++
    if __FILE__ == $0   #IF0>
        ModeleUI.ui_step("Script PrvDocuments_Last - DryRun: #{:DRYRUN}") do
            # initialisations
            ...
            #
            LOG.info("Step 3 : fin")
            puts    ModeleUI.ui_ok("Fin du script")
        end #<UI>
    end #<IF0>
