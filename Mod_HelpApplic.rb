...
#**********
    ...
    #
        ...
        #    '010' => ["*","**","***","****","*****"],
            ...
            '11'  => ["#{bold}PRV=>#{reset}BusyCal-Notion CreateEvent", "#{prv_dir}", "/Notion_CreateEvent"],
            '12'  => ["#{bold}PRV=>#{reset}PrvBudget_Calculs",          "#{prv_dir}", "/PrvBudget_Calculs",""],
            '13'  => ["#{bold}PRV=>#{reset}PrvPrintAchatsMedocs",       "#{prv_dir}", "/PrvPrintAchatsMedocs","--", %w[debug level output],],
        #    '020' => ["*","**","***","****","*****"],
            '02'  => ["#---------#{bold}=>Private files<= ▼#{reset}---------#","**","***","****","*****"],
            '20'  => ["#{bold}FIL=>#{reset}PrvUpload-File into Notion-Dossiers",    "#{prv_dir}", "PrvUpload_Dossiers4","N L N F"],
            '21'  => ["#{bold}FIL=>#{reset}PrvUpload-File from scanner",            "#{prv_dir}", "PrvUpload_Dossiers_scan","N L N F"],
        #    '030'  => ["*","**","***","****","*****"],
        ...
        #    '050' => ["*","**","***","****","*****"],
            ...
            '55'  => ["#{bold}COT=>#{reset}Process records COT",                        "#{mbr_dir}", "/EneoBwCom_MgtCot", "",],
