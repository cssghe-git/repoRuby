...
#
=begin
    Program:    EneoBwCom_ExtrOfficeExt
...
# Constants
    ...
    #filenames
    CSVFILE_OUTPUT  = "#{DIRECTORY_TO_PUT}/EXT-#{LIST_NAME}@#{Time.now.strftime("%Y-%m-%d-%H%M")}.csv"
...
#
    ...
    #
        ...
        query = {
            filter: {
                and: [
                    { property: 'CDC', select: { "does_not_equal": "NIV" } },
    ...
    #
        ...
        1.times do                                      #loop at first reason
            ...
            # membre (exterieur)
                puts    "CDC: #{prop['CDC']}"
                result['CDC Pr']= 'EXT'     if prop['CDC'].empty?
                result['code']  = 'Y'
                result['cause'] = 'Extérieur'
                list_code       = 5
                break
    ...
    #
        ...
        CSV.open(CSVFILE_OUTPUT, 'w', col_sep: ';', encoding: 'UTF-8') do |csv|
            ...
            csv << [
                ...
                'Eneo', 'EneoSport', 'V-A',
                'CDC Pr', 'CDC Sc',
                'Extérieur', 'Modification', 'Ancien', 'Décès',
                'ActPrc'
            ]
            # Données
